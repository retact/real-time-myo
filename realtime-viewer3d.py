...
import mpl_toolkits.mplot3d.axes3d import Axes3D
...
def viewer(emg_data):
    ...
    ax1.set(xlim=[0, window_size], ylim=[0, 8], zlim=[-128, 127])
    ...
    ax1.set_zlabel('EMG [-128 - 127]', fontsize=12)
    ...
    signals = []
    for i in range(8):
        signal, = ax1.plot(np.arange(window_size), np.full(window_size, i) np.zeros(window_size),alpha=0.4,zorder=8-i)
